...
def test_load_sql():
    ...
    parser._Parser__load_sql(test_sql)
    assert parser.input_sql == "SELECT * FROM table;\n"


def test_oracle_extract_variables():
...
def test_mssql_extract_variables():
    """Test that variable declarations are correctly extracted."""
    parser = mssql_parser()
    test_sql = """
...
def test_variable_replacement():
    ...
    assert parser.processing_sql.strip() == expected_sql
...
def test_summary_report():
    ...
    assert "Variable Replacemnts:  2" in report


def test_comment_new_lines():
...
Line 7"""
    ...
    assert parser.comment_suppressed_sql.strip() == output.strip()


def test_variable_handling_with_comment():
    ...
    assert parser.processing_sql.strip() == output.strip()

    parser.reset()
    parser.parse_sql(input, user_variables={"GlobalEndDate": "31-DEC-2024"})
    assert (
        parser.processing_sql.strip()
        == output.replace("02-DEC-2024", "31-DEC-2024").strip()
    )


def test_variable_handling_remove_declaration():
    parser = oracle_parser()
...
WHERE   a_column LIKE 'new' || '%'"""
    ...
    assert parser.processing_sql.strip() == output_sql.strip()


def test_variable_handling_no_remove_declaration():
    parser = mssql_parser()
    variables = {"Prefix": "new"}
    input_sql = """
use big_projects
