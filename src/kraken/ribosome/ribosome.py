...
def run(
    ...
    **kwargs: Any,
    ...
    """
    Summary:
        Takes a path (or list of paths) to SQL files or directories of SQL files, parses and executes SQL,
        and returns results:
    ...
    Args:
        filepaths (str | list[str], optional): filepath (or list of filepaths) to SQL files or directories
            of SQL files. If left blank, Kraken will search script's own directory. Defaults to "".
        ...
        export_extension (str, optional): If exporting, Kraken will export as this extension if supported.
            Defaults to "csv".
        export_filename (str, optional): If exporting, Kraken will use this filename for the export file.
            If multiple files are being exported (CSVs for multiple dataframes, for example) then Kraken will use
            this as a prefix before the dataframe name. Defaults to "".
        ...
        export_overwrite (bool, optional): If exporting, determines behaviour of saving in the event of a filename
            conflict. If a file already exists, with either overwrite or amend the filename, appending with a number
            (_01, _02, etc). Defaults to False.
        export_zip_filename (str, optional): If exporting and provided, Kraken will zip files in memory before writing
            to this filename, rather than directly. Defaults to None (direct writing of files).
        batch_size (int): Downloads rows in batches. Use `batch_size=0` to fetch all data without batching.
        delimiter (str, optional): If exporting to a delimited file format (like CSV), this is used as the delimiter.
            Defaults to ",".
        parsing_feedback (bool): If True, Kraken will print a summary report of the parsed queries. Defaults to False.
        concurrent (bool): If True, executes all SQL files concurrently. Queries within each SQL file will still execute
        ...
        isolation_level (Literal["SERIALIZABLE", "REPEATABLE READ", "READ COMMITTED", "READ UNCOMMITTED", "AUTOCOMMIT"] | None):
            ...
            with Synapse databases), try using "AUTOCOMMIT". This will override the ability to commit and rollback using the
            Connector, so this should be handled within SQL.
