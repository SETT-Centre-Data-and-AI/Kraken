...
MAIN_TEST_SCHEMA = config[integrity.SCHEMA]
...
def get_complex_dataframe():
    df = pd.DataFrame(
        {
            "a": pd.Series([1, 2, 3], dtype=np.dtype("int32")),
            "b": pd.Series(["x", "y", "z"], dtype=np.dtype("O")),
            "c": pd.Series([True, False, np.nan], dtype=np.dtype("object")),
            "d": pd.Series(["h", "i", np.nan], dtype=np.dtype("O")),
            "e": pd.Series([10, np.nan, 20], dtype=np.dtype("float")),
            "f": pd.Series([np.nan, 100.5, 200], dtype=np.dtype("float")),
            "g": pd.Series([np.nan, "071892", "171892"], dtype=np.dtype("float")),
            "h": pd.Series([True, False, np.nan], dtype=np.dtype("bool")),
            "i": pd.Series([True, False, "wor"], dtype=np.dtype("object")),
            "j": pd.Series([np.nan, np.nan, "A Long String"], dtype=np.dtype("object")),
        }
    )
    return df
...
@pytest.fixture
def dataframe_for_upload_dtype_test():
    return pd.DataFrame(
        {
            "integer_data": [0, 1],
            "float_data": [1.1, 2.1],
...
def get_test_connector(autocommit: bool = True, autoclose: bool = True):
    connector = create_connector(
        alias=MAIN_TEST_ALIAS,
        username=MAIN_TEST_USERNAME,
...
def drop_if_exists(connector: Connector, table: str):
...
def test_force_dtypes_string_mixed_nulls(mixed_type_dataframe: pd.DataFrame):
...
def test_force_dtypes_string_empty_dataframe():
    empty_df = pd.DataFrame()
...
def test_convert_dtypes_without_varchar_length(mixed_type_dataframe: pd.DataFrame):
...
def test_convert_dtypes_with_custom_varchar_length(mixed_type_dataframe: pd.DataFrame):
...
def test_convert_headers_upper(mixed_type_dataframe: pd.DataFrame):
...
def test_convert_headers_lower(mixed_type_dataframe: pd.DataFrame):
    expected_columns = ["int_column", "float_column", "str_column", "mixed_column"]
...
def test_convert_headers_none(mixed_type_dataframe: pd.DataFrame):
...
def test_convert_headers_inplace(mixed_type_dataframe: pd.DataFrame):
    expected_columns = ["INT_COLUMN", "FLOAT_COLUMN", "STR_COLUMN", "MIXED_COLUMN"]
...
def test_dtype_conversion_complex():
    df = get_complex_dataframe()
    dtype_remap = _convert_dtypes(df)
    assert dtype_remap


def test_dtype_conversion_complex_with_set_length():
    df = get_complex_dataframe()
...
def test_failure_dtype_conversion_complex_with_set_length():
    df = get_complex_dataframe()
...
def test_upload_dataframe_unstructured_df():
    ...
    with pytest.raises(UnstructuredDataFrameError):
        connector.upload(df=unstructured_df, table=table, schema=None)


### Test that a DataFrame can be uploaded
def test_upload_dataframe_real():
    df = get_mixed_type_dataframe()

    with setup_test_credentials(alias=MAIN_TEST_ALIAS, username=MAIN_TEST_USERNAME):
...
def test_failure_upload():
    ...
    with setup_test_credentials(alias=MAIN_TEST_ALIAS, username=MAIN_TEST_USERNAME):
        ...
        checker.execute(
            f"SELECT * FROM {MAIN_TEST_SCHEMA}.{TEST_UPLOAD_TABLE} WITH (NOLOCK)"
        )
...
def test_upload_table_exists(if_table_exists):
...
def test_upload_dataframe_no_dtype_specified(
    dataframe_for_upload_dtype_test,
):
    with setup_test_credentials(alias=MAIN_TEST_ALIAS, username=MAIN_TEST_USERNAME):
        connector = get_test_connector()
        connector.set_autoclose(False)
        ...
        assert set(column_data["data_type"].to_list()) == {
            "bigint",
            "float",
            "varchar",
        }
        assert column_data.query('data_type == "varchar"')["data_length"].iloc[0] == 11
...
def test_upload_dataframe_scalar_dtype(dataframe_for_upload_dtype_test):
    with setup_test_credentials(alias=MAIN_TEST_ALIAS, username=MAIN_TEST_USERNAME):
        connector = get_test_connector()
        drop_if_exists(connector, f"{MAIN_TEST_SCHEMA}.{TEST_DTYPE_TABLE}")
        ...
        column_data = connector.execute(check_columns_query(), progress=False)
        assert set(column_data["data_type"].to_list()) == {"nvarchar"}
...
def test_upload_dataframe_override_single_dtype(dataframe_for_upload_dtype_test):
    with setup_test_credentials(alias=MAIN_TEST_ALIAS, username=MAIN_TEST_USERNAME):
        connector = get_test_connector()
        connector.set_autoclose(False)
        drop_if_exists(connector, f"{MAIN_TEST_SCHEMA}.{TEST_DTYPE_TABLE}")
...
def test_upload_dataframe_dtype_and_varchar_length(dataframe_for_upload_dtype_test):
    with setup_test_credentials(alias=MAIN_TEST_ALIAS, username=MAIN_TEST_USERNAME):
        connector = get_test_connector()
        connector.set_autoclose(False)
        drop_if_exists(connector, f"{MAIN_TEST_SCHEMA}.{TEST_DTYPE_TABLE}")
        ...
        assert column_data.query('data_type == "varchar"')["data_length"].iloc[0] == 15
...
def test_complex_rollback():
    with setup_test_credentials(alias=MAIN_TEST_ALIAS, username=MAIN_TEST_USERNAME):
        ...
        uploader.execute(
            f"DROP TABLE IF EXISTS {MAIN_TEST_SCHEMA}.{TEST_UPLOAD_TABLE}", commit=False
        )
        uploader.rollback()
...
def test_complex_close_without_commit():
    with setup_test_credentials(alias=MAIN_TEST_ALIAS, username=MAIN_TEST_USERNAME):
        # Prepare
        df = get_complex_dataframe()
        dropper = get_test_connector(autocommit=True, autoclose=False)
...
def test_correct_null_uploading():
    df = pd.DataFrame(
        {
            "id": [1, 2, 3, 4],
    ...
    with setup_test_credentials(alias=MAIN_TEST_ALIAS, username=MAIN_TEST_USERNAME):
        ...
        uploader.upload(
            df,
            table=TEST_UPLOAD_TABLE,
            schema=f"{MAIN_TEST_DATABASE}.{MAIN_TEST_SCHEMA}",
        )
        check = checker.execute(
            f"""
                SELECT COUNT(*) AS count
                FROM {MAIN_TEST_DATABASE}.{MAIN_TEST_SCHEMA}.{TEST_UPLOAD_TABLE}
                WHERE  name     IS NULL
                    OR age      IS NULL
                    OR active   IS NULL"""
        )
        assert check["count"].iloc[0] == 3
...
def test_failures_raised_correctly():
    # Prepare DFs
    ...
    drop_sql = f"DROP TABLE IF EXISTS {MAIN_TEST_DATABASE}.{MAIN_TEST_SCHEMA}.{TEST_UPLOAD_TABLE}"
    create_sql = f"""
        CREATE TABLE {MAIN_TEST_DATABASE}.{MAIN_TEST_SCHEMA}.{TEST_UPLOAD_TABLE} (
                id      INT             PRIMARY KEY
